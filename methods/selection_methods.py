...
def query_samples(model, data_unlabeled, subset, direc):

    # Create unlabeled dataloader for the unlabeled subset
    unlabeled_loader = DataLoader(data_unlabeled, batch_size=1,
                                  sampler=SubsetSequentialSampler(subset),
                                  pin_memory=True)

    # Measure uncertainty of each data points in the subset
    uncertainty = get_uncertainty(model, unlabeled_loader, direc)
    arg = np.argsort(uncertainty)
    return arg

def get_uncertainty(models, unlabeled_loader, direc):
    models['backbone'].eval()
    uncertainties = []  # 使用列表存储不确定性度量

    for inputs, _ ,_ in unlabeled_loader:  # 假设标签未使用
        inputs = inputs.cuda()
        scores = models['backbone'](inputs)[0]
        posterior = F.softmax(scores, dim=1)
        loss = Categorical(probs=posterior).entropy()

        models['backbone'].zero_grad()  # 清除之前的梯度
        loss.backward()  # 计算当前样本的预测梯度
        ...
        for i in range(inputs.size(0)):  # 对每个样本
            ...
            for param in models['backbone'].parameters():
                if param.grad is not None:
                    sample_grad.append(param.grad.flatten())

            if sample_grad:
                # 合并当前样本的预测梯度
                sample_grad_tensor = torch.cat(sample_grad)
                # 计算样本预测梯度与优化方向的内积
                inner_product = torch.dot(sample_grad_tensor, direc)

                # 将内积添加到不确定性列表中
                uncertainties.append(inner_product.unsqueeze(0))

    return torch.cat(uncertainties).cpu() if uncertainties else torch.tensor([]).cpu()
...
def get_uncertainty2(models, unlabeled_loader):
    models['backbone'].eval()
    models['module'].eval()
...
def query_samples1(model, method, data_unlabeled, subset, labeled_set, cycle, args, direc):
    if method == 'Random':
